...
type BitcoinII struct{}

func init() {
	Register("bitcoinii", &BitcoinII{})
}
...
func (b *BitcoinII) TemplateRules() []string {
	return []string{"segwit"}
}

func (b *BitcoinII) ValidateAddress(address, network string) error {
	params := b.Params()
	...
	if err != nil {
		return fmt.Errorf("invalid Bitcoin II address: %s", address)
	}
	if len(payload) != 20 {
		return fmt.Errorf("invalid Bitcoin II address payload length: %d", len(payload))
	}
	if version != p2pkhVersion && version != p2shVersion {
		return fmt.Errorf("invalid Bitcoin II address version byte: 0x%02x", version)
	}

	return nil
}

func (b *BitcoinII) AddressToScript(address, network string) ([]byte, error) {
	params := b.Params()
	hrp := params.Bech32HRPMainnet
	p2pkhVersion := params.P2PKHVersionMainnet
	p2shVersion := params.P2SHVersionMainnet
...
func (b *BitcoinII) BuildCoinbase(template *noderpc.BlockTemplate, address, network, coinbaseText string,
...
func (b *BitcoinII) BuildBlock(header []byte, coinbaseTx []byte, template *noderpc.BlockTemplate) (string, error) {
	if template.DefaultWitnessCommitment != "" {
...
func (b *BitcoinII) PoolReward(template *noderpc.BlockTemplate) int64 {
