...
// Bitcoin implements the Coin interface for BTC.
type Bitcoin struct{}

func init() {
	Register("bitcoin", &Bitcoin{})
}

func (b *Bitcoin) Name() string      { return "Bitcoin" }
func (b *Bitcoin) Symbol() string    { return "BTC" }
func (b *Bitcoin) Algorithm() string { return "sha256d" }
func (b *Bitcoin) SupportsSegWit() bool { return true }

func (b *Bitcoin) Params() CoinParams {
	return CoinParams{
...
func (b *Bitcoin) TemplateRules() []string {
	return []string{"segwit"}
}

func (b *Bitcoin) ValidateAddress(address, network string) error {
	params := b.Params()
	...
	if len(payload) != 20 {
		return fmt.Errorf("invalid Bitcoin address payload length: %d", len(payload))
	}
	if version != p2pkhVersion && version != p2shVersion {
		return fmt.Errorf("invalid Bitcoin address version byte: 0x%02x", version)
	}
...
func (b *Bitcoin) BuildBlock(header []byte, coinbaseTx []byte, template *noderpc.BlockTemplate) (string, error) {
...
func (b *Bitcoin) PoolReward(template *noderpc.BlockTemplate) int64 {
