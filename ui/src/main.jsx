...
function StandaloneEngineUI() {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', background: '#020611', color: '#e2e8f0', fontFamily: 'Inter, -apple-system, sans-serif' }}>
      {/* Header */}
      <div style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '8px 16px', borderBottom: '1px solid rgba(255,255,255,0.06)', background: 'rgba(2,6,17,0.9)', flexShrink: 0, height: '42px' }}>
        ...
        <span style={{ fontSize: '13px', fontWeight: 600, color: '#e2e8f0' }}>Fleet Overview</span>
        <span style={{ fontSize: '11px', color: engineOnline ? '#00cfaa' : '#ff0080', marginLeft: '2px' }}>{engineOnline ? 'Online' : 'Offline'}</span>
      </div>
      {/* Tab bar */}
